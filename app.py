...
def register():
    ...
    try:
        ...
        cur.execute("INSERT INTO users (username, phone, password_hash) VALUES (?,?,?)",
...
def login():
    d = request.json
    conn = db(); cur = conn.cursor()
    row = cur.execute("SELECT id, username, phone FROM users WHERE (username=? OR phone=?) AND password_hash=?",
...
def group_history():
    ...
    conn = db(); cur = conn.cursor()fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])
...
if name == "__main__":
    ...
    rows = cur.execute("SELECT sender_id,message,created_at FROM group_messages WHERE group_id=? ORDER BY id",(gid,)).
